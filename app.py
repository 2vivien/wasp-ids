...
def api_login():
    ...
    if user and bcrypt.check_password_hash(user.password, password):
        ...
        with ids_capture_lock:
            if not ids_capture_attempted_start and not ids_thread_running: # Vérifie les deux flags
                app.logger.info("Première connexion ou capture non active, tentative de démarrage du thread IDS.")
                start_ids_thread_func() # Cette fonction contient déjà une vérification de ids_thread_running
                ids_capture_attempted_start = True # Marque que la tentative a eu lieu
...
def get_dashboard_critical_alerts():
    try:
        critical_logs = IDSLog.query.filter(
            IDSLog.scan_type.isnot(None),
            IDSLog.scan_type != "Journal TCP Activity",
            IDSLog.severity.isnot(None),
            IDSLog.severity.notin_(['Low', 'Faible', 'Info', 'Debug']) 
        ).order_by(IDSLog.timestamp.desc()).limit(5).all()
...
def process_packet_callback(packet_data):
    ...
    if not detection_engine_instance or not packet_data: return
        ...
        if alerts:
            with app.app_context(): 
                for alert in alerts:
                    ...
                    db_alert_data = {
                        'timestamp': alert.get('timestamp'), 'source_ip': alert.get('source_ip'),
                        'destination_ip': alert.get('destination_ip'), 'source_port': alert.get('source_port'),
                        'destination_port': alert.get('destination_port'), 'protocol': alert.get('protocol'),
                        'scan_type': alert.get('scan_type'), 'severity': alert.get('severity'),
                        'details': alert.get('details')
                    }
                    if isinstance(db_alert_data['timestamp'], str):
                        try: db_alert_data['timestamp'] = datetime.fromisoformat(db_alert_data['timestamp'].replace("Z", "+00:00"))
                        except ValueError: db_alert_data['timestamp'] = datetime.now(timezone.utc) 
                    elif not isinstance(db_alert_data['timestamp'], datetime):
                         db_alert_data['timestamp'] = datetime.now(timezone.utc) 

                    valid_data = all(db_alert_data.get(f) is not None for f in ['timestamp', 'source_ip', 'destination_ip', 'protocol'])
                    if not valid_data:
                        event_logger.error(f"Champs requis manquants pour alerte moteur DB: {alert}")
                        continue
                    
                    new_db_alert = IDSLog(**{k: v for k, v in db_alert_data.items() if v is not None or k in ['source_port', 'destination_port', 'scan_type', 'severity', 'details']})
                    try:
                        db.session.add(new_db_alert); db.session.commit()
                    except Exception as db_error:
                        db.session.rollback()
                        event_logger.error(f"Erreur DB (alerte moteur): {db_error}", exc_info=True)
    except Exception as e:
        event_logger.error(f"Erreur traitement alertes moteur: {e}", exc_info=True)
    ...
    if packet_data.get('protocol') == 'TCP':
        try:
            with app.app_context():
                ...
                timestamp_dt = packet_data.get('timestamp')
                
                if not isinstance(timestamp_dt, datetime):
                    try: timestamp_dt = datetime.fromisoformat(str(timestamp_dt).replace("Z", "+00:00"))
                    except: timestamp_dt = datetime.now(timezone.utc)

                src_info = f"{packet_data.get('source_ip')}:{packet_data.get('source_port','N/A')}"
                dst_info = f"{packet_data.get('destination_ip')}:{packet_data.get('destination_port','N/A')}"
                journal_log_formatted_string = (
                    f"[{timestamp_dt.strftime('%Y-%m-%d %H:%M:%S')}] {level_str} {model_str}: {message_str} "
                    f"(Src: {src_info}, Dst: {dst_info})"
                )
                
                severity_map = {"INFO": "Info", "WARNING": "Medium", "ERROR": "High", "DEBUG":"Low"}
                
                tcp_log_entry = IDSLog(
                    timestamp=timestamp_dt, source_ip=packet_data.get('source_ip'),
            ...
            event_logger.error(f"Erreur journalisation activité TCP: {e}", exc_info=True)
